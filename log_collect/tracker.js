...
(function () {
    ...
    var cookieUtils = {
        ...
        get: function (cookieName) {
            ...
            if (cookieText.indexOf(encodeURIComponent(cookieName)) > -1) {
                var split = cookieText.split(":");
                for (index in split) {
                    var kv = split[index].split("=");
                    var key = kv[0].trim();
                    var value = kv[1].trim();
    ...
    var tracher = {
        clinetConfig : {
            logServerUrl : "http://hadopp-1804-001/log.gif",
        ...
        isSessionTimeOut : function () {
            ...
            return currentTime - preVisitTime > this.clinetConfig.sessionTimeOut;
        },
        /**
         * 将数据发送到日志服务器上
         */
        sendDataToLogServer : function (data) {
            ...
            var url = this.clinetConfig.logServerUrl + "?" + paramsText;
            var i  = new Image(1,1);
            i.src = url;
            //更新用户最近一次访问时间
            this.updatePreVisitTime();
        ...
        setCommonColumns: function (data) {
            //sdk 版本号
            data[this.columns.version] = this.clinetConfig.logVersion;
            //用户代理信息
