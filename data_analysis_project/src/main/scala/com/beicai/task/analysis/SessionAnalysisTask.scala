...
object SessionAnalysisTask extends BaseTask{
  ...
  def loadDataFromHbase() = {
    ...
    val resultRDD = sc.newAPIHadoopRDD(jobConf,classOf[TableInputFormat],classOf[ImmutableBytesWritable],classOf[Result]).map(_._2)
    val enventLogRDD = resultRDD.map(result => {
  ...
  def sessionVisitTimeAndStepLengthAnalysisStat(eventLogRDD: RDD[(String, String, String, String, String, String, String, String, String, String, String)]) = {
    //取出需要的字段
    //tuple4RDD==>( sid, accessTime, event
    ...
    /**
      ...
      */
    val groupRDD: RDD[((String, String), Iterable[(String, String, String, String)])] = flatMapRDD.groupByKey()
    /**
   ...
   val sessionTimeAndStepLengthRDD = groupRDD.map(t2 => {
    ...
    /**
      ...
      */
    sessionTimeAndStepLengthRDD.map(t2 => {
